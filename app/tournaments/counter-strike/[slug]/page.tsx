import { getCsGoGameByTournamentByID } from '@/utils/api';
...
const Page = async ( { params }: { params: { slug: string } } ) => {
    const matchData = await getCsGoGameByTournamentByID(
        getIdFromSlug( params.slug ),
        [ 'UPCOMING', 'RUNNING' ],
        'ASC',
    );
    const resultsData = await getCsGoGameByTournamentByID(
        getIdFromSlug( params.slug ),
        [ 'PAST' ],
        'DESC',
    );
    ...
    const textVariablesMap = {
        ...
        '%TOURNAMENT_NAME%': matchData.data?.[0]?.leagueName || 'tournament',
    };
