...
export default async function Picks() {
    ...
    const dataPicks = [
        ...csPicks, ...dotaPicks, ...leaguePicks
    ];

    const { data:sidebarBannerData }: any = await getStrapiContentByPosition(
        'sidebar_picks',
    );
    // Static strings for SEO
    const pageStaticStringsApiResponse =
        await getPageStaticStrings( '/picks' );
    ...
    return (
        <div>
            <PicksPage
                dataPicks={ dataPicks }
                sidebarBannerData={ sidebarBannerData }
            />
            <SeoText
                SEO_TEXT={ SEO_TEXT }
