...
import { getCSGOMatchById, getLolMatchById } from '@/utils/api';
...
const LeagueMatch = async ( { params }: { params: { slug: string } } ) => {
    const matchIdFromSlug = getIdFromSlug( params.slug )
    const matchData = await getLolMatchById( matchIdFromSlug );
    const bestOddsValues = findMaxValuesWithNames( getCorrectOddData( matchData ) );

    const [ teamAWins, teamALooses, teamAWinRate ] = calculateWinRate( matchData?.aMatches, matchData?.teamAName ) || undefined;
    const [ teamBWins, teamBLooses, teamBWinRate ] = calculateWinRate( matchData?.aMatches, matchData?.teamAName ) || undefined;
    ...
    return (
        <>
            ...
            <SeoText SEO_TEXT={ SEO_TEXT } SEO_TEXT_HTML={ SEO_TEXT_HTML } SEO_HEADING={ SEO_HEADING } isHeadingH1={true}/>
        </>
    );
};
